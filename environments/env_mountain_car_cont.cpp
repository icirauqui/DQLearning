#include "env_mountain_car_cont.hpp"

env_mountain_car_cont::env_mountain_car_cont(bool bdbg){
    this->bDebug = bdbg;
}

env_mountain_car_cont::~env_mountain_car_cont(){}


void env_mountain_car_cont::debug_mode(bool dbg){
    this->bDebug = dbg;
}
...
void env_mountain_car_cont::step(float action, Eigen::RowVectorXf &zstate, float &zreward, bool &zdone){
    ...
    float force = action_min;
    if (action > action_min)
        force = action;
    if (force > action_max)
        force = action_max;
    
    v += (force * power) - (0.0025 * cos(3*x));
...
Eigen::RowVectorXf* env_mountain_car_cont::reset(){
    float LO = -0.6;
    float HI = -0.4;
...
std::vector<float> env_mountain_car_cont::_height(std::vector<float> xs){
    std::vector<float> ys;
...
float env_mountain_car_cont::_height(float xs){
    float ys = sin(3 * xs) * 0.45 + 0.55;
    return ys;
}


void env_mountain_car_cont::render(int framerate){
